...
impl LocalWallet {
    const VERIFY_KEY: &'static str = "verify_key";
    ...
    pub fn new<P: Into<PathBuf>>(path: P, passphrase: &str) -> Result<Self> {
        ...
        if !s.contains(Self::VERIFY_KEY) {
            let mut random = [0u8; 32];
            OsRng.fill_bytes(&mut random);
            s.encrypt(Self::VERIFY_KEY, &random, passphrase)?;
        }
    ...
    /// verify the password is true
    pub fn verify(&self, passphrase: &str) -> Result<()> {
        let _ = self.decrypt(Self::VERIFY_KEY, passphrase)?;
        Ok(())
    }
